...
public class SaOAuth2ServerController {
  ...
  public void configOAuth2Server(SaOAuth2ServerConfig oauth2Server) {
    ...
    // 配置：未登录时返回的View
    oauth2Server.notLoginView = () -> {//向第三方请求，第三方未登录的情况
      String msg = "当前会话在OAuth-Server端尚未登录，请先访问"
          + "<a href='/meta/oauth2/doLogin?name=sa&pwd=123456' target='_blank'> doLogin登录 </a>"
          + "进行登录之后，刷新页面开始授权";
      return msg;
    };
    ...
    // 配置：确认授权时返回的 view
    oauth2Server.confirmView = (clientId, scopes) -> {//第三方已经登录了，跳转到授权页面，请求授权
      String scopeStr = SaFoxUtil.convertListToString(scopes);
      String yesCode =
          "fetch('/meta/oauth2/doConfirm?client_id=" + clientId + "&scope=" + scopeStr + "', {method: 'POST'})" +
              ".then(res => res.json())" +
